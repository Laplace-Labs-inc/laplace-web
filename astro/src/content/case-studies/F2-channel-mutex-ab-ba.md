...
tags: ["case-study", "crossbeam-channel", "parking_lot", "mutex", "DPOR"]
cover:
...
DPOR found the failing schedule quickly because the channel operation is a synchronization boundary. The send makes the receive runnable, and the receive makes the consumer's second lock attempt enabled. Once both second lock attempts are enabled, the AB-BA cycle is concrete.
...
The key schedule is not large. The channel rendezvous is a hinge. Once the rendezvous happens, both second-lock operations are enabled, and neither can complete. DPOR records this as a two-resource wait-for cycle rather than as a channel stall.
